...
class LoadingBronzeData:
    ...
    def load_data(self, file_name, file_path):
        ...
        if self.mode == "append" and self.file_path_type == "child":
            ...
            df = (
                ...
                .option("header", True)
                .load(f"{file_path}/orders_{date}.csv")
            )
        else:
            df = (
                self.spark.read.format("csv")
                .option("header", True)
                .load(file_path.replace("table_name", file_name))
            )
            if self.mode == "append":
    ...
    def load_data_into_table(self, df, file_name):
        ...
        if not self.spark.catalog.tableExists(
            f"fmcg.fmcg_bronze.{self.file_path_type}_{file_name}"
        ):
            df.write.format("delta").mode("overwrite").saveAsTable(
                f"fmcg.fmcg_bronze.{self.file_path_type}_{file_name}"
            )
        else:
            df.write.format("delta").mode(self.mode).saveAsTable(
                f"fmcg.fmcg_bronze.{self.file_path_type}_{file_name}"
            )
